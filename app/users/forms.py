...
class LoginForm(FlaskForm):
    ...
    login = StringField(_('Email or Username'), validators=[DataRequired()])
...
class EditForm(FlaskForm):
    nickname = StringField(_('Nickname'),
                           validators=[Optional(), Regexp(r'[\w-]')])
    full_name = StringField(_('Full Name'), validators=[Optional()])
    email = StringField(_('Email'), validators=[Optional(), Email()])
    phone = StringField(_('Phone'), validators=[Optional()])
    zip = StringField(_('Zip'), validators=[Optional()])
    employer = StringField(_('Employer'), validators=[Optional()])
    ...
    new_password = PasswordField(_('New password'), validators=[
        Optional(), EqualTo('verify_password')
    ])
    verify_password = PasswordField(_('Verify password'),
                                    validators=[Optional()])
